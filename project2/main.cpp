...
#include <ctime>
#include <exception>
#include "functionpixel.h"
#include "ll.h"
#include "special_workman.h"
#include "boss.h"
#include "item.h"
#define ENTER '\r'
...
void menu(int current){
    ...
    while(1){
        ...
        gotoxy(5, 5);
        select(current, 1, 3); textcolor(DARKYELLOW,BLACK); cout << "1) Workman management\n";             textcolor(WHITE,BLACK);
        gotoxy(30, 10);
    	select(current, 2, 3); textcolor(YELLOW,BLACK); cout << "2) Special workman management\n";         textcolor(WHITE,BLACK);
    	gotoxy(55, 18);
        select(current, 3, 3); textcolor(GREEN ,BLACK); cout << "3) Supply management\n";           textcolor(WHITE,BLACK);
        gotoxy(80, 26);
        select(current, 4, 3); textcolor(DARKGREEN ,BLACK); cout << "4) Boss management\n";                 textcolor(WHITE,BLACK);
        gotoxy(105, 38);
        select(current, 5, 3); textcolor(AQUA, BLACK); cout << "5) See business information\n";      textcolor(WHITE,BLACK);
    	gotoxy(130, 46); 
        select(current, 6, 3); textcolor(BLUE, BLACK); cout << "6) Do noting (skip the day)\n";          textcolor(WHITE,BLACK);
        k1 = getch();
...
void enter(int x, int y){
    gotoxy(x, y);
    textcolor(RED,BLACK);
    delaycout("Press any key to enter the game...", 50); textcolor(WHITE,BLACK);
    ShowConsoleCursor(1);
    getch();
    ShowConsoleCursor(0);
    system("cls");
    ...
    switch(current){
        case 1:
            if(l.size_get() == MAX_WORKMAN){
                ...
                textcolor(RED,BLACK);
                delaycout("Result : Can't hire more workman (Max workman)\n", 50); textcolor(WHITE,BLACK);
                textcolor(VIOLET,BLACK);
                delaycout("\t\t\tPress any key to continue...", 50); textcolor(WHITE,BLACK);
                getch();
                system("cls");
            ...
            delaycout("Result : Complete hire ", 50); textcolor(WHITE,BLACK);
            delaycout(name, 50);
            delaycout("(Lv. ", 50);
            cout << lv;
            delaycout(")", 50);
            textcolor(VIOLET,BLACK);
            press_any_key(25, 40); textcolor(WHITE,BLACK);
            workman_management(1);
            break;
        case 2:
            system("cls");
...
void show_workman(int current, int current_next){
    ...
    for(int i = l.size_get()+1; i <= MAX_WORKMAN; ++i)
        printf("%3d. - Empty -\n\n", i);
    cout << ">> Command ?\n\n";
    select(current, 1, 5); cout << "Hire workman\n";
    select(current, 2, 5); cout << "Fire workman\n";
    if(current == 2){
        for(int i = 1; i <= l.size_get(); ++i){
            select(current_next, i, 9);
            l.name_display(i);
        }
        select(current_next, l.size_get()+1, 9); cout << "<< Cancel";
    }

    cout << "\n\n";
    select(current, 3, 3); cout << "<< Back\n\n";
}
...
void show_special_workman(int current, int current_next){
    ...
    cout << "  1. "; medic.display(5); cout << '\n';
    cout << "  2. "; entertainer.display(5); cout << '\n';
    cout << ">> What you want to do?\n\n";
    select(current, 1, 5); cout << "Hire special workman\n";
    hire_fire_menu(current, current_next, 1);
    select(current, 2, 5); cout << "Fire special workman\n";
...
void hire_fire_menu(int current, int current_next, int number){
    if(current == number){
        select(current_next, 1, 7); cout << "Medic\n";
        select(current_next, 2, 7); cout << "Entertainer\n";
        select(current_next, 3, 7); cout << "<< Cancel\n";
    }
}
void buy_menu(int current, int current_next, int number, int cost){
    if(current == number){
        space(9);   textcolor(AQUA,BLACK); cout << ">> Buy ?\n"; textcolor(WHITE,BLACK);
        select(current_next, 1, 11); textcolor(DARKGREEN,BLACK); cout << "Pay " << cost << " to full restock\n"; textcolor(WHITE,BLACK);
        select(current_next, 2, 11); textcolor(RED,BLACK); cout << "<< Cancel\n";                          textcolor(WHITE,BLACK);
    }
}
void buy_yes(int x, int y, string s){cout<<endl;
    gotoxy(x, y+1); printf(" _ _ _ _ _ _ _ _ _ _ _ _ _\n");
    gotoxy(x, y+2); printf("|                         |\n");
    gotoxy(x, y+3); printf("|                         |\n");
    gotoxy(x, y+4); printf("|_ _ _ _ _ _ _ _ _ _ _ _ _|\n\n");
    gotoxy(x+3, y+3); delaycout("COMPLETE RESTOCK ", 100);
    delaycout(s, 100);
}
void buy_no(int x, int y){cout<<endl;
    gotoxy(x, y); printf(" _ _ _ _ _ _ _ _ _ _ _ _ _\n");
    gotoxy(x, y+1); printf("|                         |\n");
    gotoxy(x, y+2); printf("|                         |\n");
    gotoxy(x, y+3); printf("|_ _ _ _ _ _ _ _ _ _ _ _ _|\n\n");
    gotoxy(x+2, y+2); delaycout("- OK . . . NEVERMIND  -", 100);
}
void buy_error(int x, int y){cout<<endl;
    gotoxy(x, y); printf(" _ _ _ _ _ _ _ _ _ _ _ _ _\n");
    gotoxy(x, y+1); printf("|                         |\n");
    gotoxy(x, y+2); printf("|                         |\n");
    gotoxy(x, y+3); printf("|_ _ _ _ _ _ _ _ _ _ _ _ _|\n\n");
    gotoxy(x+2, y+1); delaycout("-> Please enter Y or N", 100);
    gotoxy(x+5, y+2); delaycout("Y = YES    N = NO", 100);
}
...
void show_supply(int current, int current_next){
    ...
    cout << ">> Supply list\n\n";
    select(current, 1, 5); cout << "Soda : " << m.show_soda() << '\n';
    buy_menu(current, current_next, 1, 5000);
    select(current, 2, 5); cout << "Syrup: " << m.show_syrup() << '\n';
    buy_menu(current, current_next, 2, 7000);
    select(current, 3, 5); cout << "Ice  : " << m.show_ice() << '\n';
    buy_menu(current, current_next, 3, 2000);
    select(current, 4, 5); cout << "Cup  : " << m.show_cup() << '\n';
    buy_menu(current, current_next, 4, 9000);
    cout << "\n\n";
    select(current, 5, 3); cout << "<< Back\n\n";
}
...
void boss_management(int current){
    ...
    switch(current_next){
        case 1:
            textcolor(CYAN,BLACK);
            delaycout("Result : ", 25); textcolor(WHITE,BLACK);
            switch(current){
                case 1:
                    if(b.money_get() >= 5000){
...
void show_boss(int current, int current_next){
    ...
    b.display();
    cout << "\n>> What you want to do?\n\n";
    select(current, 1, 5); cout << "1. Buy the PPE(Personal Protective Equipment)\n";
    space(10); cout << "Effect: Reduce the chance of being Covid by 80% (1 day only)\n";
    space(10); cout << "Cost  : 5000 B\n";
    confirm_menu(current, current_next, 1);
    select(current, 2, 5); cout << "2. Cure yourself\n";
    space(10); cout << "Effect: - Recover HP to 100%\n";
    space(10); cout << "        - Reduce blood loss rate from Covid-19 by 50% (1 day only)\n";
    space(10); cout << "Cost  : 20000 B\n";
    confirm_menu(current, current_next, 2);
    select(current, 3, 5); cout << "3. Entertain yourself\n";
    space(10); cout << "Effect: - Recover MP to 100%\n";
    space(10); cout << "Cost  : 20000 B\n";
    confirm_menu(current, current_next, 3);
    cout << "\n\n";
    select(current, 4, 3); cout << "<< Back\n\n";
}
void confirm_menu(int current, int current_next, int number){
    if(current == number){

        space(11);                  textcolor(BLUE,BLACK); cout << ">> Confirm ?\n";    textcolor(WHITE,BLACK);
        select(current_next, 1, 13);textcolor(DARKGREEN,BLACK); cout << "Yes\n";                   textcolor(WHITE,BLACK);
        select(current_next, 2, 13);textcolor(RED,BLACK); cout << "<< No\n";                 textcolor(WHITE,BLACK);
    }
}
///////////
...
void skip_the_day(){
    ...
    textcolor(WHITE,BLACK);
    if(!ch) {textcolor(RED,BLACK); printf("Can't skip the day\n"); textcolor(WHITE,BLACK);} // <- check
    else{
        printf("----- Day %d -----\n\n", b.day_get());
        b.day_plus(); // day +1
        /// item
